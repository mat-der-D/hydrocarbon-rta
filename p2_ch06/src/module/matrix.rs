...
use super::{
    ...
    permutation::{calc_orbit_stabilizer, Permutable, Permutation, PermutationStore},
};
...
pub struct SaturatedHydrocarbonIter<'a, const N: usize> {
    current: AdjacencyBitMatrix<N>,
    cursor: Cursor<N>,
    seen_orbits: FxHashMap<Features<N>, FxHashSet<AdjacencyBitMatrix<N>>>,
    perm_store: &'a PermutationStore<N>,
}

impl<'a, const N: usize> SaturatedHydrocarbonIter<'a, N> {
    pub fn new(perm_store: &'a PermutationStore<N>) -> Self {
        Self {
            current: AdjacencyBitMatrix::ZERO,
            cursor: Cursor::new(),
            seen_orbits: FxHashMap::default(),
            perm_store,
        }
    }
}

impl<const N: usize> SaturatedHydrocarbonIter<'_, N> {
...
impl<const N: usize> Iterator for SaturatedHydrocarbonIter<'_, N> {
    type Item = (AdjacencyBitMatrix<N>, Vec<Permutation<N>>);

    fn next(&mut self) -> Option<Self::Item> {
        loop {
            let raw = self.next_raw()?;
            let (skeleton, feat) = raw.canonicalize();
            if self
                .seen_orbits
                .get(&feat)
                .map_or(false, |s| s.contains(&skeleton))
            {
                continue;
            }

            let perms = self.perm_store.get(&feat.make_key()).unwrap();
            let (orbit, stabilizer) = calc_orbit_stabilizer(skeleton, perms);
            self.seen_orbits.entry(feat).or_default().extend(orbit);
            return Some((skeleton, stabilizer));
        }
    }
}
