...
impl<const N: usize> Cursor<N> {
    fn new(min_irow: usize, min_icol: usize) -> Self {
        Self {
            irow: min_irow,
            icol: min_icol,
            first: (min_irow, min_icol),
        }
    }
...
impl<const N: usize> RedundantSaturatedHydrocarbonIter<N> {
    ...
    pub fn new_with_fixed_digits(fixed_digits: usize) -> Vec<Self> {
        // N * (N - 1) / 2 以上になると桁があふれるのでNG
        // N * (N - 1) / 2 - 1 以上になると, 開始時のカーソルの位置が最後の桁となり,
        // 「1回目は skip する」という仕様により壊れてしまうのでNG
        let fixed_digits = fixed_digits.min((N * (N - 1) / 2).saturating_sub(2));

        let mut iters = Vec::new();

        for bits in 0..(1u32 << fixed_digits) {
            let mut iter = Self::new();
            for i in 0..fixed_digits {
                if bits & (1 << i) != 0 {
                    iter.flip_at_cursor();
                }
                iter.cursor.move_next();
            }

            if fixed_digits > 0 {
                iter.cursor.move_prev();
                if !iter.is_fine_up_to_cursor() {
                    continue;
                }
                iter.cursor.move_next();
            }

            iter.cursor = Cursor::new(iter.cursor.irow, iter.cursor.icol);
            iters.push(iter);
        }
    ...
    fn is_fine_up_to_cursor(&self) -> bool {
        let mut prev_fat_row = 0x0004_ffff; // 直前の確定済みの fat_row
        for (i, &row) in self.current.rows.iter().enumerate() {
            let fat_row = Self::calc_fat_row(row);
            if fat_row > prev_fat_row {
                return false;
            }
            if i < self.cursor.irow || i == self.cursor.irow && self.cursor.icol == N - 1 {
                prev_fat_row = fat_row;
            }
        }
        !self.cursor.is_at_last() || self.current.is_connected()
    }
